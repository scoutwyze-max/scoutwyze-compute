...
export function registerDiscoveryRoutes(app: FastifyInstance, deps: DiscoveryRouteDeps): void {
  app.get("/llms.txt", async (_request, reply) => {
    const lines = [
      ...
      "Rail 2, x402 / USDC on Base (works on /v1/compute/rank AND /v1/route/quote), real 'exact' EVM scheme (EIP-3009 transferWithAuthorization) as of 2026-09-26: an unauthenticated request returns HTTP 402 with real payment requirements — {scheme, network, maxAmountRequired (atomic USDC units, e.g. \"150000\" = $0.15), resource, payTo, asset (the real USDC contract address), maxTimeoutSeconds, extra: {name, version} (the token's EIP-712 domain — sign under this exact domain or your authorization won't verify)}. Sign an EIP-3009 TransferWithAuthorization (from=you, to=payTo, value=maxAmountRequired, validAfter/validBefore, a random bytes32 nonce) as EIP-712 typed data, submit {x402Version:1, scheme, network, payload:{signature, authorization}} base64-encoded in the X-PAYMENT header. No on-chain broadcast from you, no ETH for gas, no API key, no signup, no card, ever — this server's facilitator (PayAI) broadcasts the settlement and pays gas; you only ever sign a message. IMPORTANT asymmetry: unlike the Bearer rail, x402 settles on successful payment verification BEFORE scoring runs — real USDC has already moved by the time a no_match/no_inventory result is known, and there is no refund path for it. Every response's billing.rail field tells you which guarantee applied. See examples/node-client.mjs or examples/python_client.py in the repo for a complete, tested reference implementation of this signing flow.",
      "On a REJECTED payment attempt (bad signature, expired/already-used authorization, insufficient amount, insufficient balance), the 402 body includes a machine-parseable `code` field alongside the human-readable `reason` — real x402/PayAI error vocabulary (invalid_payload, invalid_exact_evm_payload_signature, invalid_exact_evm_payload_authorization_value_mismatch, invalid_exact_evm_insufficient_balance, duplicate_settlement, and others — PayAI's errorReason is proxied through verbatim where recognized). `code` is a stable identifier to branch retry logic on; `reason` may change wording. `code` is absent on the very first, no-payment-submitted-yet challenge (not a failure, just the initial offer).",
      "",
      "## Endpoints",
      ...
      "/v1/route/quote": {
        post: {
          ...
          responses: {
            ...
            "402": { description: "Payment Required — no valid Bearer key and no valid X-PAYMENT header. Body includes real x402 'exact' EVM scheme payment requirements: payTo, asset (real USDC contract address), maxAmountRequired (atomic units), maxTimeoutSeconds, extra: {name, version} (EIP-712 domain — sign under this exact domain). On a rejected payment attempt (not the initial challenge), body includes a machine-parseable `code` field alongside `reason` — see /llms.txt's Auth section for the full vocabulary and a link to reference client implementations." },
          },
        },
      },
    };
