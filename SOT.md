...
**Machine-parseable failure codes (added 2026-09-25).** A rejected payment attempt (bad signature, expired/reused nonce, insufficient amount, already-used tx, RPC failure) now carries a `code` field alongside the existing free-text `reason` — real x402 spec vocabulary (`coinbase/x402/specs/x402-specification-v2.md` §9: `invalid_payload`, `invalid_exact_evm_payload_signature`, `invalid_exact_evm_payload_authorization_value_mismatch`, `invalid_transaction_state`, `unexpected_verify_error`), verified against the primary source, not guessed. Three codes are deliberately NOT from the spec — `unknown_challenge`, `challenge_already_used`, `challenge_expired` — because ScoutWyze's own pre-issued challenge/nonce layer (`ChallengeStore`) is an addition on top of base x402, not something the spec defines a vocabulary for; these are kept clearly distinct rather than force-mapped onto a spec code that would overclaim conformance. `code` is omitted (not null) on the very first, no-payment-submitted-yet challenge — that's an initial offer, not a rejected attempt, so there's nothing to code. See `X402ErrorCode` in `x402.ts` for the full type and reasoning.
...
**Registry syndication (added 2026-09-25)** — beyond the official MCP Registry (`server.json`, live since 2026-09-24), the MCP server is now also packageable as a real MCPB bundle (`mcp-server/manifest.json`, `npm run build:mcpb`) for Smithery's stdio publish path and Claude Desktop's single-click install. Real, schema-validated, and functionally verified (built bundle extracted and exercised with a live MCP `initialize`/`tools/list` handshake, not just schema-checked). **What's still manual:** Smithery requires a one-time `smithery auth login` (GitHub OAuth, human/browser step) before the CLI can actually publish — the bundle-build half is automated, the account/auth half isn't, same shape as the official registry's original GitHub OAuth namespace verification. Researched and deliberately declined: an A2A (Agent2Agent) Agent Card (real spec verified, but built for stateful task delegation this one-shot REST+x402 API has no use for — see git history 2026-09-25) and "MCPMarket" (mcpmarket.com — real and active, but a paid SEO-style directory: $29 for fast listing or a 4-6 week free queue, plain web form, no manifest format, no API — not a peer of the actual registries above).
