...
simpler integration for an Agent that already expects a single
credential in an env var, the same shape as any other API-key tool.
An x402 variant is included too (make_x402_tool) for an agent that
holds its own wallet and would rather pay per call with no signup step
at all — see python_client.py's rank_via_x402 for what that actually
does (real on-chain USDC transfer, not a mock).
"""
...
def make_bearer_tool(api_key: Optional[str] = None, base_url: str = DEFAULT_BASE_URL) -> Tool:
    ...
    server (mcp-server/) and langchain_tool.py already use."""
